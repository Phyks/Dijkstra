...
dans les Sections~\ref{sec:algor-de-dijkstra} et~\ref{sec:tas-de-fibonacci}. Un ``\emph{wrapper}''
python3 a été créé pour pouvoir interagir facilement avec
l'algorithme, comme détaille dans la Section~\ref{sec:expos-des-fonct}. Enfin, un script Python qui
lit une carte OpenStreetMap, le convertit en un format d'entrée correct pour notre implémentation et effectue des recherches dedans a été écrit.
...
elle est vraie pour toutes les itérations. On a donc trouvé la plus
...
\begin{enumerate}
  \item ajout d'une clé (la distance du nœud à la source), avec
    décoration (le nœud)
    \mint{python}|add(node, distance)|
  \item mise à jour d'une clé pour un nœud donné
    \mint{python}|add(node, new_distance)|
